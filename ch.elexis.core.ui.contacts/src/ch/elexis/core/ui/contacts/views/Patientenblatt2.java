...
public class Patientenblatt2 extends Composite implements IUnlockable {
	...
	private final static String[] lbExpandable = { Messages.Patientenblatt2_diagnosesLbl, // $NON-NLS-1$
			...
			Messages.Patientenblatt2_remarksLbk }; // $NON-NLS-1$ //$NON-NLS-2$
													// //$NON-NLS-3$
	private final Text[] txExpandable = new Text[lbExpandable.length];
	private final static String[] dfExpandable = { "Diagnosen", "PersAnamnese", //$NON-NLS-1$ //$NON-NLS-2$
	...
	void recreateUserpanel() {
		...
		fields.add(
				new InputData(Messages.Patientenblatt2_firstname, Patient.FLD_FIRSTNAME, InputData.Typ.STRING, null)); // $NON-NLS-1$
		...
		fields.add(new InputData(Messages.Patientenblatt2_sex, Patient.FLD_SEX, null,
				new String[] { Person.FEMALE, Person.MALE }, false)); // $NON-NLS-1$
		...
		String[] userfields = CoreHub.userCfg.get(CFG_EXTRAFIELDS, StringConstants.EMPTY).split(StringConstants.COMMA);
	...
	Patientenblatt2(final Composite parent, final IViewSite site) {
		...
		inpZusatzAdresse.setMenu(removeZAAction, showZAAction, copySelectedContactInfosToClipboardAction,
				copySelectedAddressesToClipboardAction);
		...
		viewmenu.createMenu(GlobalActions.printEtikette, GlobalActions.printAdresse, GlobalActions.printBlatt,
				GlobalActions.printRoeBlatt, copySelectedContactInfosToClipboardAction,
				copySelectedAddressesToClipboardAction);
	...
	public void setPatient(final Patient p) {
		...
		form.setText(StringTool.unNull(p.getName()) + StringConstants.SPACE + StringTool.unNull(p.getVorname()) + " (" //$NON-NLS-1$
